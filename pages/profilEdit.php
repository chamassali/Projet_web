...
if (isset($_SESSION['id_users'])) {
    ...
    $requser->execute(array($_SESSION['id_users']));
    $user = $requser->fetch();
    if (isset($_POST['newNom']) and !empty($_POST['newNom']) and $_POST['newNom'] != $user['nom']) {
        $newNom = htmlspecialchars($_POST['newNom']);
        $insertnom = $bdd->prepare("UPDATE users SET nom = ? WHERE id_users = ?");
        $insertnom->execute(array($newNom, $_SESSION['id_users']));
        header('Location: profil.php?id_users=' . $_SESSION['id_users']);
    }

    if (isset($_POST['newPrenom']) and !empty($_POST['newPrenom']) and $_POST['newPrenom'] != $user['prenom']) {
        $newPrenom = htmlspecialchars($_POST['newPrenom']);
        $insertprenom = $bdd->prepare("UPDATE users SET prenom = ? WHERE id_users = ?");
        $insertprenom->execute(array($newPrenom, $_SESSION['id_users']));
        header('Location: profil.php?id_users=' . $_SESSION['id_users']);
    }

    if (isset($_POST['newMail']) and !empty($_POST['newMail']) and $_POST['newMail'] != $user['mail']) {
        $newMail = htmlspecialchars($_POST['newMail']);
        $insertmail = $bdd->prepare("UPDATE users SET mail = ? WHERE id_users = ?");
        $insertmail->execute(array($newMail, $_SESSION['id_users']));
        header('Location: profil.php?id_users=' . $_SESSION['id_users']);
    }
    ...
    if (isset($_POST['newMdp1']) and !empty($_POST['newMdp1']) and isset($_POST['newMdp2']) and !empty($_POST['newMdp2'])) {
        ...
        if ($mdp1 == $mdp2) {
            $insertmdp = $bdd->prepare("UPDATE users SET motdepasse = ? WHERE id_users = ?");
            $insertmdp->execute(array($mdp1, $_SESSION['id_users']));
            header('Location: profil.php?id_users=' . $_SESSION['id_users']);
        } else {
